...
<?php foreach ($data['item'] as $row) { $stock = $this->crud->gd('master_stock', ['id' => $row->id_stock]); ?>
                                                <table style="width: 100%; border-bottom: 1px solid rgba(0,0,0,.1)">
                                                    <thead>
                                                        <tr>
                                                            <th style="width: 60%"><?=$stock->nama.' '.$row->jumlah?> buah</th>
                                                        </tr>
                                                    </thead>
                                                </table>
<?php } ?>
...
<?php } ?>
                    ...
                    <div align="center">
                        <button onclick="window.location.href = '<?=site_url('teknisi/pemasangan/sebelum/'.$data['data']->id)?>';"  style="height: 60px;width: 300px"  type="button" class="btn btn-primary warna-biru">
                            2. Proses Pemasangan >>
                        </button>
                    </div>
                </div>
