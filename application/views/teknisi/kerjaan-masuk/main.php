            ...
            <div>
                <div  style="width: 100vw" class="col-xl-12">
                    <div class="m-portlet warna-merah m-portlet--bordered-semi m-portlet--skin-dark m-portlet--full-height ">
                        <div class="m-portlet__body body_menu_1a row">
                            <div align="left"  class="col-4">
                                <span>
                                    <i style="font-size: 4rem; color: white; margin-top: 10px" class="fa fa-briefcase"></i>
                                </span>
                            </div>
...
<?php foreach ($data['data'] as $row) { ?>
                    <div  align="left" class="m-portlet m-portlet--mobile list-kerjaan" id="item-<?=strtolower($row->tipe_spk).'-'.$row->id?>" style="width: 97vw;">
                        <div class="row">
                            ...
                            <div class="col-4">
                                <button  class="btn btn-primary warna-biru" onclick="kerjakan('<?=strtolower($row->tipe_spk)?>', '<?=$row->id?>')">
                                    Kerjakan
                                </button>
                            </div>
                        </div>
                ...
                swal({
                    ...
                    preConfirm: function() {
                        return new Promise(function(resolve) {
                            $.post('<?=site_url('teknisi/kerjaan-masuk/kerjakan')?>', {
                                'tipe_spk': tipe_spk,
                                'id_spk': id_spk
